...
top-k so that the captured support is bounded. Miles asks SGLang for its native
`return_sampling_mask` response fields and uses SGLang's returned normalized
log probability as the rollout log probability.
...
- The Miles router is currently required because the native SGLang router does
  not forward the sampling-support response fields.
